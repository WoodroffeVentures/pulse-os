      ...
      <div className="w-full max-w-xl bg-[#08111f] border border-white/10 rounded-xl shadow-2xl mb-10">
        ...
        <div className="px-6 py-4 space-y-4 max-h-[75vh] overflow-y-auto">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div>
              <label className="block text-[10px] font-semibold uppercase tracking-widest text-[#617089] mb-1">Business</label>
            ...
            <div>
              <label className="block text-[10px] font-semibold uppercase tracking-widest text-[#617089] mb-1">Decision</label>
            ...
            <div>
              <label className="block text-[10px] font-semibold uppercase tracking-widest text-[#617089] mb-1">Revenue Share % (proposed)</label>
            ...
            <div className="md:col-span-2">
              <label className="block text-[10px] font-semibold uppercase tracking-widest text-[#617089] mb-1">Proposed Role</label>
            ...
            <div className="md:col-span-2">
              <label className="block text-[10px] font-semibold uppercase tracking-widest text-[#617089] mb-1">Proposed Contribution</label>
            ...
            {form.decision === 'Join with Conditions' && (
              <div className="md:col-span-2">
                <label className="block text-[10px] font-semibold uppercase tracking-widest text-[#617089] mb-1">Conditions</label>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              <label className="text-[10px] font-semibold uppercase tracking-widest text-[#617089]">Milestones</label>
            ...
            {milestones.map((m, i) => (
              <div key={i} className="flex items-center gap-2">
                <input value={m.title} onChange={e => setMilestone(i, 'title', e.target.value)}
                  className="flex-1 bg-[#020912] border border-white/10 rounded px-3 py-1.5 text-xs text-[#E6EDF5] placeholder:text-[#374151] focus:outline-none focus:border-[#C6A66B]/40"
                ...
                <button onClick={() => removeMilestone(i)} className="text-[#617089] hover:text-red-400"><X className="w-3.5 h-3.5" /></button>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="space-y-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            {decision && (
              <span className={`text-[10px] font-semibold px-2 py-0.5 rounded border uppercase tracking-widest ${DECISION_STYLES[decision] ?? 'text-[#617089] bg-white/5 border-white/10'}`}>
                {decision}
              </span>
            )}
          </div>
        ...
        <button onClick={() => setExpanded(v => !v)} className="text-[#617089] hover:text-[#E6EDF5] flex-shrink-0">
      ...
      {expanded && (
        <div className="space-y-4 pt-3 border-t border-white/10">
          {r.evidence?.proposed_role && (
            <div><div className="text-[10px] uppercase tracking-widest text-[#617089] mb-1">Role</div><p className="text-xs text-[#9BA7B8]">{r.evidence.proposed_role}</p></div>
          )}
          {r.evidence?.proposed_contribution && (
            <div><div className="text-[10px] uppercase tracking-widest text-[#617089] mb-1">Contribution</div><p className="text-xs text-[#9BA7B8]">{r.evidence.proposed_contribution}</p></div>
          )}
          ...
          {r.evidence?.viability_score !== undefined && (
            <div className="text-xs text-[#617089]">Viability score at decision: {r.evidence.viability_score}</div>
          )}

          {milestones.length > 0 && (
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-widest text-[#617089] mb-2">Milestones</div>
              <div className="space-y-2">
                {milestones.map(m => (
                  <div key={m.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className={`text-xs ${m.completed ? 'text-[#617089] line-through' : 'text-[#E6EDF5]'}`}>{m.title}</p>
                      {m.target_date && <p className="text-[10px] text-[#374151]">{m.completed ? `Completed ${new Date(m.completed_at!).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short' })}` : `Target: ${new Date(m.target_date).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' })}`}</p>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {audit.length > 0 && (
            <div>
              <div className="text-[10px] font-semibold uppercase tracking-widest text-[#617089] mb-2">Audit Timeline</div>
              <div className="space-y-1.5">
                {audit.map((a, i) => (
                  <div key={i} className="flex items-start gap-2 text-[10px]">
                    <Clock className="w-3 h-3 text-[#374151] flex-shrink-0 mt-0.5" />
                    <span className="text-[#617089]">{new Date(a.at).toLocaleString('en-ZA', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</span>
          ...
          <div className="text-[10px] text-[#374151] font-mono">Record ID: {r.id.slice(0, 8)}…</div>
        </div>
      )}
    </div>
  );
      ...
      {!fetching && records.length === 0 && (
        <div className="flex flex-col items-center justify-center h-48 gap-3 rounded-xl border border-dashed border-white/10">
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-[#617089] mt-1">Run a viability assessment first, then record your decision here</p>
          </div>
