...
const URGENCY_STYLES: Record<string, string> = {
  ...
  normal:   'text-[#9BA7B8] bg-white/5 border-white/10',
  low:      'text-[#9BA7B8] bg-white/5 border-white/5',
};
...
export default function OpportunityWorkspacePage() {
  ...
  if (orgLoading || loading) return <div className="p-6 text-[#9BA7B8] text-sm">Loading workspace…</div>;
  ...
  return (
    <div className="space-y-0">
      ...
      <div className="pb-4 border-b border-white/10 space-y-3">
        <button onClick={() => router.push('/opportunities')} className="flex items-center gap-1.5 text-xs text-[#9BA7B8] hover:text-[#C6A66B] transition-colors">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1 min-w-0 space-y-2">
            ...
            <div className="flex flex-wrap items-center gap-3 text-[11px] text-[#9BA7B8]">
          ...
          {opp.readiness_score !== null && (
            <div className="flex-shrink-0 text-right">
              <div className="text-xs text-[#9BA7B8] mb-0.5">Readiness</div>
        ...
        <div className="flex items-center gap-2">
          {editDecision !== '' ? (
            <div className="flex items-center gap-2 flex-1">
              ...
              <button onClick={() => setEditDecision('')} className="text-[#9BA7B8] hover:text-[#E6EDF5]"><X className="w-3.5 h-3.5" /></button>
            </div>
          ) : (
            <div className="flex items-center gap-2">
              <AlertCircle className="w-3.5 h-3.5 text-amber-400 flex-shrink-0" />
              <span className="text-xs text-[#9BA7B8]">
                {opp.next_decision ? <>Decision required: <strong className="text-amber-400">{opp.next_decision}</strong></> : <span className="text-[#9BA7B8]">No decision recorded</span>}
              </span>
              <button onClick={() => setEditDecision(opp.next_decision ?? '')} className="ml-1 text-[10px] text-[#9BA7B8] hover:text-[#C6A66B] flex items-center gap-0.5">
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-4 py-3 text-xs font-semibold whitespace-nowrap border-b-2 transition-colors ${
              tab === t.key
                ...
                : 'border-transparent text-[#9BA7B8] hover:text-[#9BA7B8]'
            }`}
          >
      ...
      <div className="pt-6 space-y-4">
          ...
          <div className="space-y-4">
            {opp.description && (
              <div className="bg-[#08111f] border border-white/10 rounded-lg p-4 space-y-1">
                <div className="text-[10px] font-semibold uppercase tracking-widest text-[#9BA7B8]">Description</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {Object.entries({
                ...
                <div key={k} className="bg-[#08111f] border border-white/10 rounded-lg p-3 space-y-1">
                  <div className="text-[10px] font-semibold uppercase tracking-widest text-[#9BA7B8]">{k}</div>
            ...
            {(opp.eligibility?.evidence_requirements as unknown as string[])?.length > 0 && (
              <div className="bg-[#08111f] border border-white/10 rounded-lg p-3">
                <div className="text-[10px] font-semibold uppercase tracking-widest text-[#9BA7B8] mb-2">Evidence Requirements</div>
                <ul className="space-y-1">
                  {(opp.eligibility?.evidence_requirements as unknown as string[]).map((r, i) => (
                    <li key={i} className="text-xs text-[#9BA7B8] flex items-start gap-1.5"><ChevronRight className="w-3 h-3 text-[#9BA7B8] mt-0.5 flex-shrink-0" />{r}</li>
                  ))}
                </ul>
              </div>
            )}
            <div className="text-[9px] text-[#9BA7B8] tracking-widest pt-2">
          ...
          <div className="space-y-4">
            ...
            {viability.length === 0 ? (
              <div className="flex flex-col items-center gap-3 py-12 rounded-xl border border-dashed border-white/10">
                <Zap className="w-8 h-8 text-[#9BA7B8]" />
                ...
                <div key={va.id} className="bg-[#08111f] border border-white/10 rounded-lg p-4 space-y-3">
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      ...
                      <div className="text-[10px] text-[#9BA7B8] mt-0.5">
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-[10px] font-semibold px-2 py-0.5 rounded border uppercase tracking-widest ${
                      ...
                      : va.recommendation === 'Hold' ? 'text-[#9BA7B8] bg-white/5 border-white/10'
                  ...
                  {Array.isArray(va.evidence) && va.evidence.length > 0 && (
                    <div className="space-y-1.5">
                      <div className="text-[10px] font-semibold uppercase tracking-widest text-[#9BA7B8]">Evidence factors</div>
                      {va.evidence.map((e: any, i: number) => (
                        <div key={i} className="flex items-center gap-2 text-xs">
                          {e.met ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" /> : <Circle className="w-3.5 h-3.5 text-[#9BA7B8] flex-shrink-0" />}
                          <span className={e.met ? 'text-[#9BA7B8]' : 'text-[#9BA7B8]'}>{e.label}</span>
                          <span className="ml-auto text-[10px] text-[#9BA7B8]">{e.weight}%</span>
                        </div>
                      ))}
                    </div>
                  )}
                  {Array.isArray(va.actions) && va.actions.length > 0 && (
                    <div className="space-y-1">
                      <div className="text-[10px] font-semibold uppercase tracking-widest text-[#9BA7B8]">Next actions</div>
                      {va.actions.map((a: any, i: number) => (
                        <div key={i} className="text-xs text-[#9BA7B8] flex items-start gap-1.5">
                          <ChevronRight className="w-3 h-3 text-[#9BA7B8] mt-0.5 flex-shrink-0" />{a}
                        </div>
                      ))}
                    </div>
                  )}
        ...
        {tab === 'participants' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-xs text-[#9BA7B8]">{participation.length} participation record{participation.length !== 1 ? 's' : ''}</p>
              ...
              participation.map(pr => (
                <div key={pr.id} className="bg-[#08111f] border border-white/10 rounded-lg p-4 space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    {pr.evidence?.decision && (
                      <span className={`text-[10px] font-semibold px-2 py-0.5 rounded border uppercase tracking-widest ${
                        ...
                        : pr.evidence.decision === 'Hold' ? 'text-[#9BA7B8] bg-white/5 border-white/10'
                        : 'text-red-400 bg-red-400/10 border-red-400/20'
                      }`}>
                        {pr.evidence.decision}
                      </span>
                  ...
                  {Array.isArray(pr.evidence?.milestones) && pr.evidence.milestones.length > 0 && (
                    <div className="space-y-1">
                      <div className="text-[10px] font-semibold uppercase tracking-widest text-[#9BA7B8]">Milestones</div>
                      {pr.evidence.milestones.map((m: any) => (
                        <div key={m.id} className="flex items-center gap-2 text-xs">
                          {m.completed ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 flex-shrink-0" /> : <Clock className="w-3.5 h-3.5 text-[#9BA7B8] flex-shrink-0" />}
                          <span className={m.completed ? 'text-[#9BA7B8] line-through' : 'text-[#9BA7B8]'}>{m.title}</span>
                          {m.target_date && <span className="ml-auto text-[10px] text-[#9BA7B8]">{new Date(m.target_date).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short' })}</span>}
                        </div>
                      ))}
                    </div>
                  )}
        ...
        {tab === 'activation' && (
          <div className="space-y-4">
            <div className="bg-[#08111f] border border-white/10 rounded-lg p-4 space-y-2">
              <div className="text-[10px] font-semibold uppercase tracking-widest text-[#9BA7B8]">Activation State</div>
            ...
            {participation.flatMap(pr => (pr.evidence?.milestones ?? []).map((m: any) => ({
              ...
              <div className="space-y-2">
                <div className="text-[10px] font-semibold uppercase tracking-widest text-[#9BA7B8]">All milestones</div>
                {participation.flatMap(pr => (pr.evidence?.milestones ?? []).map((m: any) => ({
                  ...m,
                  business: (pr as any).business_profiles?.business_name,
                }))).map((m: any, i: number) => (
                  <div key={i} className="flex items-center gap-3 bg-[#08111f] border border-white/10 rounded-lg px-4 py-2.5">
                    {m.completed ? <CheckCircle2 className="w-4 h-4 text-emerald-400 flex-shrink-0" /> : <Clock className="w-4 h-4 text-[#9BA7B8] flex-shrink-0" />}
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-medium text-[#E6EDF5]">{m.title}</div>
                      {m.business && <div className="text-[10px] text-[#9BA7B8]">{m.business}</div>}
                    </div>
                    {m.target_date && <span className="text-[10px] text-[#9BA7B8]">{new Date(m.target_date).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short' })}</span>}
                  </div>
                ))}
              </div>
            ) : (
              <div className="py-8 text-center text-sm text-[#9BA7B8]">No milestones recorded yet.</div>
            )}
          </div>
        )}
        ...
        {tab === 'outcomes' && (
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <p className="text-xs text-[#9BA7B8]">Outcomes linked to this organisation's records</p>
            ...
            {outcomes.length === 0 ? (
              <div className="flex flex-col items-center gap-3 py-12 rounded-xl border border-dashed border-white/10">
                ...
                <p className="text-xs text-[#9BA7B8]">Record an outcome after milestones are completed</p>
              </div>
            ) : (
              outcomes.map(o => (
                <div key={o.id} className="bg-[#08111f] border border-white/10 rounded-lg p-4 space-y-2">
                  ...
                  <div className="text-[10px] text-[#9BA7B8]">{new Date(o.created_at).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
                </div>
              ))
            )}
          </div>
        ...
        {tab === 'evidence' && (
          <div className="space-y-4">
            ...
            {viability.length > 0 ? (
                ...
                <div key={va.id} className="bg-[#08111f] border border-white/10 rounded-lg p-4 space-y-3">
                  ...
                  {Array.isArray(va.evidence) && va.evidence.map((e: any, i: number) => (
                    <div key={i} className="flex items-start gap-2 text-xs border-b border-white/5 pb-2 last:border-0 last:pb-0">
                      {e.met ? <CheckCircle2 className="w-3.5 h-3.5 text-emerald-400 mt-0.5 flex-shrink-0" /> : <AlertCircle className="w-3.5 h-3.5 text-[#9BA7B8] mt-0.5 flex-shrink-0" />}
                      <div className="flex-1 min-w-0">
                        <div className={e.met ? 'text-[#9BA7B8]' : 'text-[#9BA7B8]'}>{e.label}</div>
                        {e.note && <div className="text-[10px] text-[#9BA7B8] mt-0.5">{e.note}</div>}
                      </div>
                      <span className="text-[10px] text-[#9BA7B8] flex-shrink-0">{e.weight}%</span>
                    </div>
                  ))}
                  <ConfidenceBadge confidence={va.confidence} />
                </div>
              ...
              <div className="py-8 text-center text-sm text-[#9BA7B8]">
